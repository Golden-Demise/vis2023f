...
function _s(FileAttachment){return(
FileAttachment("output.json").json()
)}

function _simple(s){return(
s.children.at(14)
)}
...
function _Strong(d3,simple,drag,invalidation)
{
  const width = 1000;
  const height = 1200;
  ...
  const node = svg.append("g")
      ...
      .on("mouseenter", (event, d) => {
        if(d.data.leval === 2 || d.data.leval === 3){
          ...
          const yOffset = -(newWidth - circleRadius * size_offset) / 2;
      ...
      .text(d => {
        if(d.data.leval == 1){
          ...
          return "系所："+d.data.Department+
                 ...
                 "\n分作業2成績："+d.data.Hw2_score+
                 "\n分作業3成績："+d.data.Hw3_score+
                 "\n分作業4成績："+d.data.Hw4_score+
                 "\n分作業5成績："+d.data.Hw5_score+
                 "\n分作業6成績："+d.data.Hw6_score+
                 "\n分作業7成績："+d.data.Hw7_score+
                 "\n分作業8成績："+d.data.Hw8_score+
                 "\n分作業9成績："+d.data.Hw9_score+
                 "\n分作業10成績："+d.data.Hw10_score+"分";
        }
      }
  );
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("s")).define("s", ["FileAttachment"], _s);
  main.variable(observer("simple")).define("simple", ["s"], _simple);
  main.variable(observer("drag")).define("drag", ["d3"], _drag);
  main.variable(observer("Strong")).define("Strong", ["d3","simple","drag","invalidation"], _Strong);
  return main;
}
